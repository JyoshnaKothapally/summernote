...
export default class Handle {
  ...
  update(target, event) {
    ...
    const isImage = dom.isImg(target);
	 var isVideo = dom.isVideo(target);  //Added Video Option
  ...
  if (isVideo) {
      ...
      const origVideoObj = new video();
      origVideoObj.src = $video.attr('src');

      const sizingText = videoSize.w + 'x' + videoSize.h + ' (' + this.lang.video.original + ': ' + origVideoObj.width + 'x' + origImageObj.height + ')';
      $selection.find('.note-control-selection-info').text(sizingText);
      this.context.invoke('editor.saveTarget', target);
